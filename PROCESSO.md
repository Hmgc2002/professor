...
| 2026-09-17 | cassete-dados, cassete-captura | Dei como «confirmados» o realizador, o ano e a duração de um documentário a partir de **uma** ficha do Letterboxd, que é agregada de bases de dados de terceiros. Estavam errados dois de três: um só realizador (o outro nome é o fotógrafo) e 87 minutos, não 80. E a discrepância de duração que eu próprio registei ficou em aberto em vez de ser resolvida | Para uma obra, a ficha confere-se na **fonte primária** — o sítio da obra, o editor ou a distribuidora — e não num agregador. Uma discrepância entre fichas é uma pergunta por responder, não uma nota de rodapé: resolve-se abrindo a fonte primária antes de publicar |
| 2026-09-17 | (infraestrutura) | O passo 6 do `validar.sh` acusava «deck de secretária» como termo sensível: o `bash` corria o `grep -w` com o locale C, onde o «á» são dois bytes que não são letra, e via «secret». O mesmo `grep`, experimentado à mão no zsh, **não** o apanhava — o que me levou primeiro a uma explicação errada | `LC_ALL=C.UTF-8` no `grep`, com o falso e o verdadeiro positivo testados. Regra: um passo do validador experimenta-se **na shell em que o validador corre**, não na do terminal |
| 2026-09-18 | astrofotografia | Escrevi as saídas de **sete exercícios** antes de os correr, com o carimbo «Saída (corrida a …)» já posto. Seis dos sete estavam errados. É a **quinta** vez que este padrão aparece neste registo, e desta vez aconteceu num tópico cujo gerador de lições já executava todos os blocos do texto principal: a disciplina estava aplicada à prosa das lições e **não** aos exercícios | As saídas dos exercícios passaram a ser coladas de execuções, como as do texto. A lição de fundo, outra vez: um mecanismo que só cobre parte do material deixa a outra parte com a confiança do mecanismo e nenhuma da verificação |
| 2026-09-18 | astrofotografia | A fotometria de uma pilha contava o ruído de Poisson do astro **N vezes a mais**, porque uma pilha é uma média e não uma soma. O resultado dizia 75 % abaixo do previsto — e eu **já tinha escrito a explicação plausível** (a mediana custa ruído, o sigma-clip rejeita, a interpolação correlaciona). A explicação não era falsa; era irrelevante, e teria passado por revisão | Uma explicação plausível para um resultado errado é mais perigosa do que um resultado sem explicação. O que a apanhou foi haver um número previsto ao lado. Ficou como a lição 9 do tópico e como uma regra executável: o `projeto.py` recusa-se a correr sem `PREVISOES.md` |
| 2026-09-18 | astrofotografia | Duas figuras passaram o `validar_figuras.mjs` e estavam más: as barras de variância **não estavam à escala** — a do escuro desenhada três vezes maior do que devia, na figura cuja mensagem é «a do escuro quase não se vê» — e uma seta atravessava um rótulo. Quinta ocorrência desta família | O verificador mede geometria (fora do `viewBox`, sobreposição, figura vazia); **não mede se o desenho diz a verdade**. Uma barra fora de escala é um erro de conteúdo com forma de figura, e só olhar apanha. ⚠️ Fica por decidir se é automatizável: exigiria o verificador saber que valores a figura afirma |
| 2026-09-18 | astrofotografia | A rotação por digest das posições das respostas certas (D-036, posta depois de três falhas) passou o limiar do `validar.py` **por sorte**: 29 de 60 na «c», 48 %, quando o aviso dispara aos 50 % | Tirar o período não é o mesmo que equilibrar a contagem. As posições passaram a ser construídas equilibradas e depois baralhadas com semente fixa: 20/20/20 por construção. Regra: quando um validador tem limiar, a correção não é ficar do lado bom do limiar — é tornar a propriedade verdadeira |
| 2026-09-18 | (infraestrutura) | Tentei instalar o `tidy` para o passo 2 do `validar.sh` e a única compilação disponível nesta máquina era de 2015, experimental, sem conhecimento de `<main>` nem de SVG: acusava 14 problemas numa página do almanaque que já tinha passado | Não se instalou. «Um validador com falsos positivos não se volta a correr» também se aplica **antes** de o instalar. O passo ficou declarado como por correr — na nota das fases, no `CHANGELOG.md` e em D-055 — e correu-se em substituição um verificador de equilíbrio de etiquetas, dizendo o que ele **não** faz |
| 2026-09-18 | astrofotografia | O heredoc do `bash` desta sessão comeu as barras invertidas: um remendo em Python com a sequência barra-invertida-n dentro de uma cadeia de caracteres escreveu **mudanças de linha a sério** no ficheiro, partindo-o ao meio — e a própria linha deste registo saiu partida da mesma maneira à primeira tentativa | Os remendos com escapes passaram a ser feitos com a ferramenta de edição ou com ficheiros escritos por inteiro, e a prosa dos programas passou a usar `print()` vazio em vez de escapes no fim das cadeias. Regra: um remendo cujo conteúdo tem escapes verifica-se **a correr o ficheiro**, não a ler o remendo |
